...
public class KeyStoreTest extends TestBase {
	...
	static OSSKeyStore _store = null;
	static {
		try {
			_store = new OSSKeyStore(TEST_RESOURCES_PATH + "testStore.bks",
					"testpass");
	...
	public void testLoadCertificateFromFile() throws Exception {
		...
		Certificate cert = _store.loadCertificateFromFile(TEST_RESOURCES_PATH
				+ TEST_CERTIFICATE);
		assertNotNull(cert);
		FileInputStream fileStream = new FileInputStream(TEST_RESOURCES_PATH
				+ TEST_CERTIFICATE);
